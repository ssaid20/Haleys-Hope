...
router.get("/olderCtoppResults/:testId", rejectUnauthenticated, (req, res) => {
  ...
  pool
    ...
    .catch((err) => {
      console.error("Error completing SELECT older_ctopp query for test id", err);
      res.sendStatus(500);
    });
});
...
router.put("/:student_id/:id", rejectUnauthenticated, (req, res) => {
  ...
  for (let key in updatedOCtopp) {
    if (updatedOCtopp.hasOwnProperty(key) && key !== "student_id" && key !== "id") {
      updateFields.push(`"${key}" = $${valueCount}`);
      values.push(updatedOCtopp[key]);
      valueCount++;
    }
  }
  ...
  const queryText = `UPDATE "older_ctopp" SET ${updateFields.join(
    ", "
...
router.delete("/:student_id/:id", rejectUnauthenticated, (req, res) => {
  ...
  const queryText = 'DELETE FROM "older_ctopp" WHERE "student_id" = $1 AND "id" = $2';
  pool
